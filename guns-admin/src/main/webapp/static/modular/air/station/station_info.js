...
StationInfoDlg.addSubmit = function() {
    ...
    this.stationInfoData['areaId']=$("#pullDownTreeCurIdone").val();
...
StationInfoDlg.editSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/air/station/update", function(data){
        Feng.success("修改成功!");
        window.parent.Station.table.refresh();
        StationInfoDlg.close();
    },function(data){
 ...
 * 弹出地图选择框
 */
StationInfoDlg.layerAmap = function(){
	var area = $("#pullDownTreeCurIdone");
	
	if($.trim(area.val())=="" || area.val() == 0){
	...
	var url=Feng.ctxPath+'/area/detail/'+parseInt($.trim(area.val()));
	$.get(url,function(result){
		if(result){
			if(result.type!='5'){
				...
				//根据位置搜索
				var area_select=$('#pullDownTreeCurNameone').html().trim();
				...
				var index = layer.open({
		            ...
		            shadeClose:true,
		            offset:'10px',
		            content: Feng.ctxPath + '/air/station/layerMap?address='+area_select 
		        });
